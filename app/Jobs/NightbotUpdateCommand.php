...
class NightbotUpdateCommand implements ShouldQueue
{
    ...
    public function handle()
    {
        try {
            ...
            if ($summary->playback_status != "Playing") {
                $message = "Nothing from Spotify is playing right now.";
            }
    
            $api = new NightbotAPI($accessToken);

            $commands = $api->getCustomCommands();
            $command = $commands->firstWhere('name', '!song');
            if (empty($command)) {
                $command = $api->addCommand(30, 'Please wait while we gather information.', '!song', 'everyone');
            }

            $api->editCommand($command->id, $message);
